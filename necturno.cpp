...
const float FRICCION_SUELO_NEPTUNO = 0.01f; 
const float TEMPERATURA_AMBIENTE_NEPTUNO = -218.0f; 
...
public:
    ...
    ObjetoFisico(float x, float y, float mass, float radius, float elasticity = 0.8f)
        : position(x, y), velocity(0, 0), acceleration(0, 0), angularVelocity(0, 0), 
          mass(mass), radius(radius), elasticity(elasticity), angle(0), temperature(TEMPERATURA_AMBIENTE_NEPTUNO) {
        momentInercia = 0.5f * mass * radius * radius;
    }

    void aplicarGravedad() {
        acceleration.y += GRAVEDAD_NEPTUNO;  
    }

    void aplicarFriccion() {
        if (position.y <= PISO_Y + radius && velocity.magnitude() > 0) {
            // Fricción atmosférica en Neptuno
            Vector2D friccion = velocity;
            friccion.normalize();
            velocity = velocity - friccion * FRICCION_SUELO_NEPTUNO;
        }
    }
