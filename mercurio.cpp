...
const float FRICCION_SUELO = 0.2f;      
const float TEMPERATURA_AMBIENTE_MERCURIO = 430.0f; 
const float TEMPERATURA_MIN_MERCURIO = -180.0f;  
...
public:
    ...
    ObjetoFisico(float x, float y, float mass, float radius, float elasticity = 0.8f)
        : position(x, y), velocity(0, 0), acceleration(0, 0), angularVelocity(0, 0), 
          mass(mass), radius(radius), elasticity(elasticity), angle(0), temperature(TEMPERATURA_AMBIENTE_MERCURIO) {
        momentInercia = 0.5f * mass * radius * radius;
    }

    void aplicarGravedad() {
        acceleration.y += GRAVEDAD_MERCURIO; 
    }
