...
const float PRESION_PLUTON = 1.0e-5f; 
const float RADIO_PLUTON = 1188.0f;    
...
public:
    ...
    ObjetoFisico(float x, float y, float mass, float radius, float elasticity = 0.8f)
        : position(x, y), velocity(0, 0), acceleration(0, 0), angularVelocity(0, 0), 
          mass(mass), radius(radius), elasticity(elasticity), angle(0), temperature(TEMPERATURA_PLUTON) {
        momentInercia = 0.5f * mass * radius * radius;
    }

    void aplicarGravedad() {
        acceleration.y += GRAVEDAD_PLUTON;  
    }

    void aplicarTemperatura() {
        if (position.magnitude() < RADIO_PLUTON) { 
            temperature = TEMPERATURA_PLUTON; 
        }
    }
