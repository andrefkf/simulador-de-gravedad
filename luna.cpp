...
const float GRAVEDAD_LUNA = 1.63f; 
const float DELTA_TIME = 0.016f;     
const float PISO_Y = 0.0f;           
const float FRICCION_SUELO = 0.1f;   
const float TEMPERATURA_ESPACIO = -270.0f; // Temperatura del espacio en grados Celsius (-270°C)
const float TEMPERATURA_CRITICA = -100.0f;
...
public:
    ...
    ObjetoFisico(float x, float y, float mass, float radius, float elasticity = 0.8f)
        : position(x, y), velocity(0, 0), acceleration(0, 0), angularVelocity(0, 0), 
          mass(mass), radius(radius), elasticity(elasticity), angle(0), temperature(TEMPERATURA_ESPACIO), 
          congelado(false) {
        momentInercia = 0.5f * mass * radius * radius;
    }

    void aplicarGravedad() {
        acceleration.y += GRAVEDAD_LUNA;  
    }
    ...
    void enfriar() {
        if (!congelado) {
            temperature += (TEMPERATURA_ESPACIO - temperature) * 0.01f; // Baja la temperatura hacia el frío espacial

            if (temperature <= TEMPERATURA_CRITICA) {
                congelado = true;
                elasticity = 0.2f;
                FRICCION_SUELO = 0.05f;
            }
        }
    }
...
int main() {
    ...
    for (int i = 0; i < 1000; ++i) {
        ...
        // Mostrar posiciones y temperatura
