...
const float FRICCION_SUELO = 0.1f;      
const float RESISTENCIA_AIRE = 0.47f;  
const float DENSIDAD_AIRE = 1.225f;     
const float TEMPERATURA_AMBIENTE = 25.0f; 
const float AREA_FRONTAL = 0.0314f;     
...
public:
    ...
    ObjetoFisico(float x, float y, float mass, float radius, float elasticity = 0.8f)
        : position(x, y), velocity(0, 0), acceleration(0, 0), angularVelocity(0, 0), 
          mass(mass), radius(radius), elasticity(elasticity), angle(0), temperature(TEMPERATURA_AMBIENTE) {
        momentInercia = 0.5f * mass * radius * radius;
    }

    void aplicarGravedad() {
        acceleration.y += GRAVEDAD_TIERRA;  // Aplicar gravedad terrestre
    }
    ...
    void aplicarFriccion() {
        if (position.y <= PISO_Y + radius && velocity.magnitude() > 0) {
            ...
            friccion.normalize();
            velocity = velocity - friccion * FRICCION_SUELO;
        }
    }

    void aplicarResistenciaAire() {
        if (velocity.magnitude() > 0) {
            float areaFrontal = AREA_FRONTAL; 
            float velocidadRelativa = velocity.magnitude();
            float fuerzaArrastre = 0.5f * RESISTENCIA_AIRE * DENSIDAD_AIRE * areaFrontal * std::pow(velocidadRelativa, 2);

            Vector2D direccionResistencia = velocity;
            direccionResistencia.normalize();

            acceleration = acceleration - direccionResistencia * (fuerzaArrastre / mass);
        }
    }
