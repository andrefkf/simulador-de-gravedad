...
const float FRICCION_SUELO_VENUS = 0.3f;  
const float TEMPERATURA_AMBIENTE_VENUS = 462.0f; 
...
public:
    ...
    ObjetoFisico(float x, float y, float mass, float radius, float elasticity = 0.8f)
        : position(x, y), velocity(0, 0), acceleration(0, 0), angularVelocity(0, 0), 
          mass(mass), radius(radius), elasticity(elasticity), angle(0), temperature(TEMPERATURA_AMBIENTE_VENUS) {
        momentInercia = 0.5f * mass * radius * radius;
    }

    void aplicarGravedad() {
        acceleration.y += GRAVEDAD_VENUS;  
    }
    ...
    void aplicarFriccion() {
        if (position.y <= PISO_Y + radius && velocity.magnitude() > 0) {
            ...
            friccion.normalize();
            velocity = velocity - friccion * FRICCION_SUELO_VENUS;
        }
    }
